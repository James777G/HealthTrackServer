...
public interface UserCredentialMapper {
    ...
    @Select("select id, username, password, email FROM users_credentials WHERE username = #{username}")
    UserCredential getUserCredentialByUsername(@Param("username") String username);


    @Insert("insert into users_credentials (id, username, password, email) values (#{id}, #{username}, #{password}, #{email})")
